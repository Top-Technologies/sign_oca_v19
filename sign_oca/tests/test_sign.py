...
class TestSign(BaseCommon):
    ...
    def test_template_generate_filename_defaults_to_template_name(self):
        self.configure_template()
        f = Form(
            self.env["sign.oca.template.generate"].with_context(
                default_template_id=self.template.id
        ...
        self.assertEqual(f.filename, self.template.name)
        action = f.save().generate()
        request = self.env[action["res_model"]].browse(action["res_id"])
        self.assertEqual(request.filename, self.template.name)

    def test_template_field_required_defaults_true(self):
        self.configure_template()
        self.assertTrue(self.template.item_ids.required)
    ...
    def test_auto_sign_template_refuse(self):
        self.configure_template()
        f = Form(
            self.env["sign.oca.template.generate"].with_context(
                default_template_id=self.template.id, default_sign_now=True
        ...
        signer.request_id.action_send(sign_now=True)
        signer.refuse()
        self.assertFalse(self.env["sign.oca.request"].browse(request_id).exists())
