...
class SignOcaRequest(models.Model):
    ...
    dms_directory_id = fields.Many2one("dms.directory", string="DMS Directory")
    ...
    def _compute_signer_id(self):
        ...
        for record in self:
            ...
            filtered_with_signature = user_diff_roles.filtered(lambda x: x.is_allow_signature)
            record.signer_id = (
                filtered_with_signature[0]
                if filtered_with_signature
                else user_diff_roles[0] if user_diff_roles else False
            )
    ...
    def _check_signed(self):
        ...
        if all(self.mapped("signer_ids.signed_on")):
            # Move to signed state
            self.state = "2_signed"

            # Persist signed PDF once (create attachment and a Documents/DMS entry)
            try:
                # avoid duplication: check for existing attachments for this record
                ...
                name_str = self.name or 'Signed Document'
                base_name = name_str if name_str.lower().split('.')[-1] != 'pdf' else name_str[:-4]
                target_name = base_name
                counter = 1
                while True:
                    collision = self.env['ir.attachment'].sudo().search([
                        ('name', '=', "%s.pdf" % target_name),
                    ], limit=1)
                    if not collision:
                        break
                    counter += 1
                    target_name = "%s (%s)" % (base_name, counter)
                new_name = "%s.pdf" % target_name

                # prepare datas as base64 string
                datas_value = self.data.decode() if isinstance(self.data, bytes) else self.data
                ...
                # Try creating a Documents (Enterprise) record linked to the attachment
                if 'documents.document' in self.env:
                    try:
                        ...
                        # best-effort: set folder/directory if available on template
                        tpl = self.template_id if hasattr(self, 'template_id') else None
                        if tpl and hasattr(tpl, 'dms_directory_id') and tpl.dms_directory_id:
                            if 'directory_id' in self.env['documents.document']._fields:
                                doc_vals['directory_id'] = tpl.dms_directory_id.id
                            elif 'folder_id' in self.env['documents.document']._fields:
                                doc_vals['folder_id'] = tpl.dms_directory_id.id
                        self.env['documents.document'].sudo().create(doc_vals)
                    except Exception:
                        _logger.exception('Failed creating documents.document for signed file')
                # Fallback to OCA DMS
                elif 'dms.file' in self.env:
                    try:
                        content_value = datas_value
                        # determine target directory: template setting or any available directory
                        tpl = self.template_id if hasattr(self, 'template_id') else None
                        dir_id = False
                        if self.dms_directory_id:
                            dir_id = self.dms_directory_id.id
                        elif tpl and getattr(tpl, 'dms_directory_id', False):
                            dir_id = tpl.dms_directory_id.id
                        else:
                            # fallback to first available directory (best-effort)
                            directory = self.env['dms.directory'].sudo().search([], limit=1)
                            dir_id = directory.id if directory else False
                        if not dir_id:
                            _logger.error('No DMS directory found; skipping dms.file creation for %s', new_name)
                        else:
                        ...
                        _logger.exception('Failed creating dms.file for signed file')
            except Exception:
                _logger.exception('Failed to persist signed document')
...
class SignOcaRequestSigner(models.Model):
    ...
    def refuse(self, access_token=False):
        self.ensure_one()
        if self.signed_on:
            raise ValidationError(
                self.env._("Users %s has already signed the document")
        ...
        if self.request_id.state != "0_sent":
            raise ValidationError(self.env._("Request cannot be refused"))
